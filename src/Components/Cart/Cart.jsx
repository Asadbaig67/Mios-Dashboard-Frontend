...
const Cart = () => {
  ...
  const subTotalD = (cart) => {
    ...
    cart?.forEach((item) => {
      subTotal +=
        item.product.discountedPriceD > 0
          ? item.product.discountedPriceD * item.quantity
          : item.product.dropshipperPrice * item.quantity;
    });
    return subTotal;
  };
  ...
  return (
    <>
      ...
      <div className="text-end">
        <Link to="/">
          <button className="btn btn-info mb-2 me-1">
            Add More Items <AddIcon className=" ms-1" />{" "}
          </button>
        </Link>
      </div>
