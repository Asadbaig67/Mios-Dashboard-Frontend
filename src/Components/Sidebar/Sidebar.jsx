        ...
        <>
            {(!(location.pathname.includes('admin')) && window.innerWidth <= 750) ?
                <Box sx={{ display: 'flex' }}>
                    ...
                    {loading ? null : <Navbar position="fixed" open={open}>
                        <Toolbar>
                    ...
                    <Drawer anchor="left" PaperProps={{ style: { backgroundColor: 'transparent', backdropFilter: 'blur(5px)' } }} BackdropProps={{ invisible: false, style: { opacity: 1, backgroundColor: "transparent" }, }} open={open} onClose={handleDrawerClose}>
                        ...
                        <div style={{ backgroundColor: 'transparent', height: "100vh", width: "300px" }}>
                            <ul>
                                ...
                                {arr.map((item, ind) => {
                                    return (
                                        <li key={ind} id={item.path} className='AdminSideBarLink py-1' style={location.pathname.toLowerCase() === item.path.toLowerCase() ? { backgroundColor: 'deepskyblue', color: 'white' } : null} onClick={Navigation}>
                                            &nbsp;&nbsp;{item.title}
                                        </li>
                                    )
                                })}
                                ...
                                <div>
                                    ...
                                    {orderStatuses.map((item, ind) => {
                                        return (
                                            <p key={ind} id={item.path} className="AdminSidebarSubHead m-0 py-1" style={location.pathname.toLowerCase() === item.path.toLowerCase() ? { backgroundColor: 'deepskyblue', color: 'white' } : null} onClick={Navigation}>
                                ...
                                <div>
                                    ...
                                    {navArr.map((item, ind) => {
                                        return (
                                            <p id={item.path} key={ind} className="AdminSidebarSubHead m-0 py-1" style={location.pathname.toLowerCase() === item.path.toLowerCase() ? { backgroundColor: 'deepskyblue', color: 'white' } : null} onClick={Navigation}>
                                                &nbsp;&nbsp;{item.title}
                                            </p>)
                                    })}
                                </div>
                                ...
                                {/* Categories */}
                                <div>
                                    <h6 style={{ paddingTop: "10px", paddingLeft: "6px", }}>Categories</h6>
                                    {categories.map((item, ind) => {

                                        return (
                                            <Typography onClick={() => { handleDrawerClose() }}>
                                                <Link key={ind} to={`/category/${item._id}`} >
                                                    <p className="AdminSidebarSubHead m-0 py-1 px-2" style={location.pathname.toLowerCase() === `/category/${item._id}` ? { backgroundColor: 'deepskyblue', color: 'white' } : null} >
                        ...
                        <div style={{ backgroundColor: 'transparent', height: "100vh", marginTop: "2vh", width: "200px" }}>
                            ...
                            <ul>
                                ...
                                {(location.pathname !== "/" && location.pathname.toLowerCase() !== "/featured" && location.pathname.toLowerCase() !== "/onsale" && location.pathname.toLowerCase() !== "/instock" && location.pathname.toLowerCase() !== "/outofstock" && !location.pathname.includes("/category/")) && arr.map((item, ind) => {
                                    return (
                                        <li key={ind} id={item.path} className='AdminSideBarLink my-0 py-1' style={location.pathname.toLowerCase() === item.path.toLowerCase() ? { backgroundColor: 'deepskyblue', color: 'white' } : null} onClick={Navigation}>
                                            &nbsp;&nbsp;{item.title}
                                        </li>)
                                })}
                                <Divider />
                                ...
                                {(location.pathname !== "/" && location.pathname.toLowerCase() !== "/featured" && location.pathname.toLowerCase() !== "/onsale" && location.pathname.toLowerCase() !== "/instock" && location.pathname.toLowerCase() !== "/outofstock" && !location.pathname.includes("/category/")) && <div>
                                    ...
                                    {orderStatuses.map((item, ind) => {
                                        ...
                                        return (
                                            <p key={ind} id={item.path} className="AdminSidebarSubHead my-0 py-0" style={location.pathname.toLowerCase() === item.path.toLowerCase() ? { backgroundColor: 'deepskyblue', color: 'white' } : null} onClick={Navigation}>
                                                &nbsp;&nbsp;{item.title}({numbers[item.title.toLowerCase()]})
                                            </p>)
                                    })}
                                    <Divider />
                                ...
                                <div>
                                    {(location.pathname === "/" || location.pathname.toLowerCase() !== "/featured" || location.pathname.toLowerCase() !== "/onsale" || location.pathname.toLowerCase() !== "/instock" || location.pathname.toLowerCase() !== "/outofstock" || location.pathname.includes("/category/")) && <div>
                                        ...
                                        {categories.map((item, ind) => {
                                            return (
                                                <Link key={ind} to={`/category/${item._id}`}  >
                                                    <p className="AdminSidebarSubHead mt-0 mb-0 px-2" style={location.pathname.toLowerCase() === `/category/${item._id}` ? { backgroundColor: 'deepskyblue', color: 'white', } : null}>
